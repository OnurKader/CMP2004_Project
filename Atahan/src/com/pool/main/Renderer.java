...
public class Renderer
{
	...
	public void draw_image(Image image, int x, int y, double scale) {
		if(x < -image.get_width()*scale || y < -image.get_height()*scale || x > pixel_width || y > pixel_height) { return; }
	...
	public void draw_image(Image image, int x, int y, double scale, double angle) {
		if(x < -image.get_width()*scale || y < -image.get_height()*scale || x > 2*pixel_width*scale || y > 2*pixel_height*scale) { return; }
	...
	public void draw_image_tile(ImageTile image, int x, int y, int t_x, int t_y, double scale) {
		if(x < -image.get_tile_width() || y < -image.get_tile_height() || x > pixel_width || y > pixel_height) { return; }

		int _x = 0, _y = 0;
		int _width = (int) (image.get_tile_width() * scale), _height = (int) (image.get_tile_height() * scale);
		...
		for(int j = _y; j < _height; j++) {
			for(int i = _x; i < _width; i++) {
				for(int n = 0; n < scale; n++) {
					set_pixel(x + i + n, y + j, image.get_pixels()[(int)((i/scale + t_x * image.get_tile_width()) + (j/scale + t_y * image.get_tile_height()) * image.get_width())]);
				}
			}
		}
	}
